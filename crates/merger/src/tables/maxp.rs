...
use crate::strategies::{first, max};
...
pub fn merge_maxp(fonts: &[FontRef], total_glyphs: u16) -> Result<Maxp> {
    ...
    if is_version_1 {
        ...
        let max_contours: Vec<u16> = tables
            .iter()
            .map(|t| t.max_contours().unwrap_or(0))
            .collect();
        let max_composite_points: Vec<u16> = tables
            .iter()
            .map(|t| t.max_composite_points().unwrap_or(0))
            .collect();
        ...
        let max_twilight_points: Vec<u16> = tables
            .iter()
            .map(|t| t.max_twilight_points().unwrap_or(0))
            .collect();
        let max_storage: Vec<u16> = tables
            .iter()
            .map(|t| t.max_storage().unwrap_or(0))
            .collect();
        let max_function_defs: Vec<u16> = tables
            .iter()
            .map(|t| t.max_function_defs().unwrap_or(0))
            .collect();
        let max_instruction_defs: Vec<u16> = tables
            .iter()
            .map(|t| t.max_instruction_defs().unwrap_or(0))
            .collect();
        let max_stack_elements: Vec<u16> = tables
            .iter()
            .map(|t| t.max_stack_elements().unwrap_or(0))
            .collect();
        ...
        let max_component_elements: Vec<u16> = tables
            ...
            .collect();
        let max_component_depth: Vec<u16> = tables
            .iter()
            .map(|t| t.max_component_depth().unwrap_or(0))
            .collect();
