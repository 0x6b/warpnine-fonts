...
pub fn merge_hhea(fonts: &[FontRef], num_h_metrics: u16) -> Result<Hhea> {
    ...
    let advance_width_maxs: Vec<u16> = tables
        .iter()
        .map(|t| t.advance_width_max().to_u16())
        .collect();
    let min_lsbs: Vec<i16> = tables
        .iter()
        .map(|t| t.min_left_side_bearing().to_i16())
        .collect();
    let min_rsbs: Vec<i16> = tables
        .iter()
        .map(|t| t.min_right_side_bearing().to_i16())
        .collect();
